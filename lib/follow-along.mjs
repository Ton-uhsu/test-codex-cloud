...
export function openFollow(exercise, { onComplete = null, onClose = () => {}, assetBase = './assets/exercises/' } = {}) {
  ...
  dialog.innerHTML = `<div class="follow-top"><div><p class="eyebrow">ทำไปพร้อมกัน · ทดลอง 1 ท่า</p><h2 id="follow-title">${h(exercise.name)}</h2></div><button type="button" class="secondary" data-close aria-label="ปิดโหมดทำไปพร้อมกัน">ปิด ×</button></div>
    <video data-loop playsinline muted preload="none" poster="${h(assetBase)}lateral-raise.jpg" aria-label="ภาพสาธิตยกดัมเบลออกด้านข้าง"><source src="${h(assetBase)}lateral-raise.mp4" type="video/mp4"></video>
