...
import { createFollow, stepFollow, followRemaining, followTargets } from './follow-state.mjs';
...
export function openFollow(exercise, { onComplete = null, onClose = () => {}, assetBase = './assets/exercises/', autoStart = false } = {}) {
  ...
  dialog.innerHTML = `<div class="follow-top"><div><p class="eyebrow">ทำไปพร้อมกัน</p><h2 id="follow-title">${h(exercise.name)}</h2></div><button type="button" class="secondary" data-close aria-label="ปิดโหมดทำไปพร้อมกัน">ปิด ×</button></div>
    ...
    <div class="follow-readout"><span data-count>0 / ${targets[0]} จังหวะ</span><strong data-cue role="status" aria-live="polite">ดูวิธีเล่น แล้วเลือกจำนวนครั้ง</strong></div>
    ...
    <div class="follow-options"><label>จังหวะต่อเซ็ต <select data-reps>${targets.map(reps => `<option value="${reps}">${reps} ครั้ง</option>`).join('')}</select></label><button type="button" class="secondary" data-voice-test>ทดลองเสียงไทย</button><button type="button" class="text-button" data-mute aria-pressed="false">ปิดเสียง</button></div>
    <p class="small-note" data-voice-status></p>
    <div class="button-row"><button type="button" class="action" data-start>เตรียมตัว 5 วินาที แล้วเริ่ม</button><button type="button" class="secondary" data-pause hidden>หยุดพัก</button><button type="button" class="action" data-confirm hidden>${onComplete ? 'ไปบันทึกจำนวนครั้งที่ทำจริง' : 'ฉันทำครบเซ็ตแล้ว → พัก'}</button><button type="button" class="secondary" data-extend hidden>พักเพิ่ม 30 วินาที</button></div>
  ...
  let s = createFollow(targets[0]), muted = false, closed = false, generation = 0, clock, audio;
  ...
  function begin() {
    if (document.hidden || !['idle', 'paused', 'error', 'done'].includes(s.phase)) return;
    if (s.phase === 'done') s = createFollow(s.reps);
    unlockAudio(); video.load(); lastPrep = -1;
    s = stepFollow(s, 'start'); paint();
  }
  dialog.addEventListener('change', event => {
    const reps = Number(event.target.value);
    if (event.target.matches('[data-reps]') && s.phase === 'idle' && targets.includes(reps)) { s = createFollow(reps); paint(); }
  });
