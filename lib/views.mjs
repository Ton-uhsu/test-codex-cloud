import { DAYS, WEEK_ORDER, MUSCLES, escapeHTML as h, safeSourceURL, filterExercises, workoutForDay, nextTrainingDay } from './guide.mjs?v=3';
import { youtubeLinks, videoPreview } from './video.mjs?v=3';
...
function tips(data) {
  return `<aside class="aside" aria-label="คำแนะนำก่อนฝึก">
    ...
    <section class="tip-card"><p class="tip-index">BEFORE YOU LIFT</p><h2>วอร์มอัป แล้วค่อยเริ่ม</h2>
      <p>เดินหรือขยับร่างกายเบา ๆ สักครู่ก่อนฝึก เคลียร์พื้นให้ไม่ลื่น ดูคลิปแล้วลองทำมือเปล่าก่อนถือดัมเบล หากยังไม่มั่นใจให้ผู้ฝึกสอนช่วยตรวจท่า</p>
...
export function todayView(data, state) {
  ...
  const session = workout ? `<section aria-labelledby="session-title">
    <div class="session-banner"><span class="session-letter" aria-hidden="true">${h(workout.key)}</span>
      <div><h2 id="session-title">โปรแกรม ${h(workout.key)} · ${h(workout.name)}</h2><p>${workout.exercises.length} ท่า · ดัมเบล + น้ำหนักตัว · ทำที่บ้านได้</p></div>
    </div>
    <p>${h(workout.description)} ดูคลิปก่อนเริ่ม แล้วทำทีละท่าให้ครบเซ็ตก่อนเปลี่ยนท่า</p>
  ...
  return heading('DUMBBELLS AT HOME', day === today ? 'วันนี้ เล่นอะไรดี?' : `แผนวัน${DAYS[day]}`, 'มีดัมเบลก็เริ่มได้ ดูคลิป แล้วฝึกไปทีละท่า', 'มือใหม่ · 2 วัน / สัปดาห์') + `
    ...
    <p class="equipment-note">${h(data.program.equipmentNote)}</p>
