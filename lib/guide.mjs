...
export function viewFromHash(hash) {
  ...
  return ['home', 'plan', 'setup', 'nutrition', 'today', 'library', 'guide'].includes(view) ? view : 'home';
}
...
export function validateGuide(data) {
  ...
  require(data && data.schemaVersion === 2, 'schema version');
  ...
  for (const exercise of data.exercises) {
    ...
    require(Array.isArray(exercise.equipmentTypes) && exercise.equipmentTypes.length > 0 &&
      exercise.equipmentTypes.every((type) => ['dumbbell', 'bodyweight', 'floor', 'wall'].includes(type)), 'home equipment');
    require(exercise.video && typeof exercise.video.youtubeId === 'string' && /^[A-Za-z0-9_-]{11}$/.test(exercise.video.youtubeId) &&
      ['title', 'channel', 'sourceUrl', 'checkedAt'].every((key) => hasText(exercise.video[key])) && exercise.video.language === 'en', 'exercise video');
    safeSourceURL(exercise.video.sourceUrl);
    require(exercise.video.checkedAt === data.reviewedAt, 'video reference date');
    require(exercise.learningNote === undefined || hasText(exercise.learningNote), 'learning note');
  }
  const program = data.program;
  require(program && ['name', 'audience', 'provenance', 'doseNote', 'restNote', 'equipmentNote'].every((key) => hasText(program[key])), 'program fields');
